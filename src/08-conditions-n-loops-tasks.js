...
function findFirstSingleChar(str) {
  ...
  for (let i = 0; i < charArr.length; i += 1) {
    ...
    for (let j = 0; j < charArr.length; j += 1) {
      if (current === charArr[j] && i !== j) has = true;
    }
  ...
  for (let i = 0; i < arr.length; i += 1) {
    switch (arr[i]) {
      ...
      case '<':
        ...
        break;
      case '>':
        if (res[res.length - 1] !== 4) return false;
        res.pop();
        break;
      case '}':
        if (res[res.length - 1] !== 3) return false;
        res.pop();
        break;
      case ']':
        if (res[res.length - 1] !== 2) return false;
        res.pop();
        break;
      case ')':
        if (res[res.length - 1] !== 1) return false;
        res.pop();
        break;
      default:
        break;
